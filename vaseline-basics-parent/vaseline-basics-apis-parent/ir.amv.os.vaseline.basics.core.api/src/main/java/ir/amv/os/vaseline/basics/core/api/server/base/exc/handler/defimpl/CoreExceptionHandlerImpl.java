package ir.amv.os.vaseline.basics.core.api.server.base.exc.handler.defimpl;
...
public class CoreExceptionHandlerImpl implements ICoreExceptionHandler {
	...
	public BaseVaselineClientException convertException(Exception exception) {
		...
		BaseVaselineClientException clientException;
		try {
			clientException = exceptionHandler.convertException(exception);
		} catch (ExceptionConversionException e) {
			clientException = new BaseVaselineClientException("Problem converting exception " + e.getMessage());
		}
